...
| `gatecheck.py` | nothing | Clicking Explore asks for a username and the shared password, refuses each half distinctly, and lets any domain through with the right password. The one check that does not take `chrome()`'s sign-in bypass. |
