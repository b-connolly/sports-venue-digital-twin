...
seventy-six thousand people deciding at one whistle, onto a network already
carrying the city's ordinary Sunday. It is the harder problem, it is what venue
operations actually loses sleep over, and it is invisible in a model that stops
at the property line.

`E` draws CDOT's own published count segments around the venue, coloured by
`VCRATIO` — volume over capacity at the design hour, so 1.0 is not "busy", it is
the point at which the road has run out. Width carries the volume, colour
carries how full: a fat red line is a big road with nothing left. The finding is
that there is no spare network. I-25 is at 1.17 and US 6 at 1.05 before a single
car leaves the north lot; Colfax is 0.97, Sheridan 0.91, US 287 0.93.

The arithmetic beside it is deliberately the kind you can check in your head — a
published capacity, an occupancy figure that is **stated on screen rather than
hidden**, and the vehicle count that falls out of the two. Somebody who
disagrees with 2.6 people to a car can see exactly which number to argue with,
which is the difference between an estimate and an assertion. A panel reading
"estimated clearance: 47 minutes" would be more impressive and would be a
fabrication: there is no model here, there is a count from a state DOT and a
division, and it says so.

What it is **not** is game-day traffic. AADT is an annual average of all days
and CDOT publishes no hourly series here, so the honest claim is about the
baseline — what the roads normally carry, and how little of it is spare. The
panel is labelled that way. Anything stronger needs NPMRDS or CDOT's continuous
count stations, which are a data-sharing agreement rather than a download.

`data/traffic.json` is generated by `tools/build_traffic.py` and committed, for
the same reason the replays are: Pages serves straight from the repository, and
a demo should not depend on a state DOT's server being up on the day. Sixty-five
segments, 26 kB. CDOT's own server carries the current year; the copy mirrored
to ArcGIS Online was six years behind when this was written, which is why the
generator asks the source.

The `FeatureLayer` is imported on demand and the counts are fetched on first
open, so a viewer who never presses `E` pays nothing for it. The lines sit eight
metres above ground rather than draped on it — the captures are above the
terrain surface, so a line on the terrain is a line underneath the roads it is
describing.
...
| Keyboard | `←` `→` views · `H` reset · `M` analysis · `T` time of day · `C` copy camera · `E` getting out · `U` hide interface |
